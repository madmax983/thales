...
impl StrategyConfig for PviTrendConfig {}

pub struct PviTrend {
    config: PviTrendConfig,
}

impl PviTrend {
    pub fn new(config: PviTrendConfig) -> Self {
        Self { config }
    }
}

#[async_trait]
impl Strategy for PviTrend {
    fn name(&self) -> &str {
        "PviTrend"
    }
    ...
    async fn generate_signals(&self, data: &DataFrame) -> Result<Vec<Signal>> {
        ...
        let close_arr = close_str_series.str()?;

        let time_series = data.column("timestamp_unix_ms")?;
        let time_arr = time_series.cast(&DataType::Int64)?;
        let time_arr = time_arr.i64()?;

        // Calculate PVI. It returns a series of Strings.
        let pvi_series = pvi::calculate(data)?;
        let pvi_arr = pvi_series.str()?;

        // PVI as f64 to calculate SMA
        let pvi_f64_series = pvi_series.cast(&DataType::Float64)?;

        // Calculate SMA of PVI (Signal line)
        // We have to put PVI in a dataframe for SMA to read the "close" column
        let mut pvi_cloned = pvi_f64_series.clone();
        pvi_cloned.rename("close");
        let temp_df = DataFrame::new(vec![pvi_cloned])?;
        let signal_series = sma::calculate(&temp_df, self.config.pvi_sma_period)?;
        ...
        for i in 1..close_arr.len() {
            ...
            let pvi_curr_s = pvi_arr.get(i);
            let pvi_prev_s = pvi_arr.get(i - 1);

            let sig_curr = signal_arr.get(i);
            let sig_prev = signal_arr.get(i - 1);

            let price_s_opt = close_arr.get(i);
            ...
            if let (
                ...
                // Long Entry
                if p_prev <= s_prev && p_curr > s_curr {
                    ...
                    signals.push(Signal {
                        ...
                        reason: "PVI crossed above Signal Line (Bullish Trend)".to_string(),
                ...
                if p_prev >= s_prev && p_curr < s_curr {
                    ...
                    signals.push(Signal {
                        ...
                        stop_loss: Some(sl.to_f64().unwrap_or(0.0)),
                        take_profit: Some(tp.to_f64().unwrap_or(0.0)),
                        reason: "PVI crossed below Signal Line (Bearish Trend)".to_string(),
                        timestamp_ms: timestamp,
                    });
                }
                ...
                if p_prev >= s_prev && p_curr < s_curr {
                    signals.push(Signal {
                        ...
                        stop_loss: None,
                        take_profit: None,
                        reason: "PVI crossed below Signal Line (Long Exit)".to_string(),
                        timestamp_ms: timestamp,
                    });
                }
                ...
                if p_prev <= s_prev && p_curr > s_curr {
                    signals.push(Signal {
                        ...
                        stop_loss: None,
                        take_profit: None,
                        reason: "PVI crossed above Signal Line (Short Exit)".to_string(),
                        timestamp_ms: timestamp,
                    });
                }
            }
    ...
    async fn update_params(&mut self, params: serde_json::Value) -> Result<()> {
        let new_config: PviTrendConfig = serde_json::from_value(params)?;
...
mod tests {
    ...
    async fn test_empty_data() -> Result<()> {
        let config = PviTrendConfig::default();
    ...
    async fn test_parameter_validation() -> Result<()> {
        let mut strategy = PviTrend::new(PviTrendConfig::default());
        ...
        assert_eq!(strategy.config.pvi_sma_period, 100);
