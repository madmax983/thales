...
/// Configuration for the VWAP (Volume Weighted Average Price) Reversion Strategy.
///
...
impl VwapReversion {
    /// Creates a new instance of the VWAP (Volume Weighted Average Price) Reversion Strategy.
    ///
    /// # Examples
    ///
    /// ```rust
            ...
            ) = (price_opt, prev_price_opt, vwma_opt, prev_vwma_opt, atr_opt)
            {
                ...
                if prev_price_dec >= prev_lower_band && price_dec < lower_band {
                    ...
                    signals.push(Signal {
                        ...
                        reason: format!(
                            "VWAP (Volume Weighted Average Price) Reversion: Price {:.2} < Lower Band {:.2}",
                            price, lower_band
                        ),
                ...
                // Sell when price goes above the upper band (overbought)
                    ...
                    signals.push(Signal {
                        ...
                        take_profit: Some(tp.to_f64().unwrap_or(0.0)),
                        reason: format!(
                            "VWAP (Volume Weighted Average Price) Reversion: Price {:.2} > Upper Band {:.2}",
                            price, upper_band
                        ),
                        timestamp_ms: timestamp,
                    });
                ...
                if prev_price_dec < prev_vwma_dec && price_dec >= vwma_dec {
                    ...
                    signals.push(Signal {
                        ...
                        take_profit: None,
                        reason: format!(
                            "VWAP (Volume Weighted Average Price) Reverted to Mean: Price {:.2} >= VWMA {:.2}",
                            price, vwma_val
                        ),
                        timestamp_ms: timestamp,
                    });
