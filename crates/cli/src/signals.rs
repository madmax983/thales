...
///
/// A tuple containing:
/// 1. A vector of `TradeIntent` objects representing valid trading opportunities.
/// 2. A vector of warning strings explaining why signals were skipped or filtered.
pub async fn generate_signals(
...
) -> Result<(Vec<TradeIntent>, Vec<String>)> {
    ...
    if valid_signals.is_empty() {
        if raw_signals_count > 0 {
            ...
            let msg = "No signals generated by the strategy.".to_string();
            eprintln!("{}", msg);
            warnings.push(msg);
        }
    }
    ...
    if let Some(signal) = valid_signals.first() {
        ...
        if signals_today < 3 {
            ...
            // DEBUG logs removed or moved to warnings if critical,
            // but these seem like pure debug info.
                ...
                && existing_pos.is_none()
            {
                skip = true;
                let msg = format!(
                    "Skipping Exit signal for {} because no open position exists.",
                    signal.symbol
                );
                // eprintln!("{}", msg); // Optional: print to stderr
                warnings.push(msg);
            }
...
mod tests {
    ...
    async fn test_generate_signals_macd_integration() -> Result<()> {
        ...
        // Strategy Name "Macd"
        let (intents, _) =
            generate_signals(&series, "Macd", None, 100.0, &positions, None).await?;
    ...
    async fn test_generate_signals_filter_invalid() -> Result<()> {
        ...
        assert!(
            warnings.iter().any(|w| w.contains("invalid size")),
            "Should warn about invalid size"
        );
    ...
    async fn test_daily_signal_limit() -> Result<()> {
        ...
        let (intents, _) = generate_signals(
            &series,
            "BollingerBands",
        ...
        let (intents_2, _) = generate_signals(
            &series,
            "BollingerBands",
    ...
    async fn test_historical_performance_adjustment() -> Result<()> {
        ...
        let (intents_low, _) = generate_signals(
            &series,
            "BollingerBands",
    ...
    async fn test_generate_signals_filter_chasing() -> Result<()> {
        ...
        let (intents_skipped, warnings) = generate_signals(
            &series,
            "BollingerBands",
            None,
            100.0,
        ...
        assert!(
            warnings.iter().any(|w| w.contains("Overbought")),
            "Should warn about Overbought"
        );
        ...
        let (intents_skipped_sell, warnings) = generate_signals(
            &series_sell,
            "BollingerBands",
            None,
            100.0,
        ...
        assert!(
            warnings.iter().any(|w| w.contains("Oversold")),
            "Should warn about Oversold"
        );
        ...
        let (intents_allowed, _) = generate_signals(
            &series,
            "BollingerBands",
            None,
            100.0,
    ...
    async fn test_generate_signals_low_confidence() -> Result<()> {
        ...
        let (intents, warnings) = generate_signals(
            &series,
            "BollingerBands",
            None,
            100.0,
        ...
        assert!(
            warnings.iter().any(|w| w.contains("low confidence")),
