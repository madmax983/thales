...
fn validate_intent(intent: &TradeIntent) -> Vec<String> {
    ...
    if let Some(ot) = &intent.order_type {
        match ot.as_str() {
            "limit" => {
            ...
            "stop" => {
                if intent.stop_price.is_none() {
                    errors.push("stop_price is required for stop orders".to_string());
                }
            },
            "stop_limit" => {
                if intent.limit_price.is_none() || intent.stop_price.is_none() {
                    errors.push("limit_price and stop_price are required for stop_limit orders".to_string());
                }
            },
            "market" => {}, // OK
            _ => errors.push(format!("unknown order_type: {}", ot)),
        }
    }
