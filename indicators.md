...
**Rationale:** Used as a faster moving average than the simple or exponential moving average. It reduces lag by adding the difference between the EMA and a smoothed EMA back to the original EMA.

### Implementation Details
- Uses `rust_decimal::Decimal` for precision internally during calculations.
- Uses two successive passes of the `ema` indicator on the "close" column.
- Calculates DEMA using the formula: `(2 * EMA1) - EMA2`.
- Returns a Polars `Series` of `f64` values representing the DEMA.
...
### Parameters
- `data`: Reference to a Polars `DataFrame`. Must contain a numeric "close" column.
- `period`: The lookback period (typically 14 or 21).
