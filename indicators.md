...
**Name:** DEMA
**Description:** Calculates the Double Exponential Moving Average, an indicator designed to reduce the lag of traditional EMAs, making it more responsive to price changes.
**Rationale:** Used as an alternative to simple or exponential moving averages for trend identification and crossover signals, providing a faster response to price changes.

### Implementation Details
- Uses `rust_decimal::Decimal` for precision internally as required by constraints.
- Uses two successive passes of the `ema` indicator on the "close" column to compute DEMA = (2 * EMA1) - EMA2.
...
- The first `period * 2` (approximate) values will be null.
