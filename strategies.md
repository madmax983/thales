...
**Rationale:** Moving averages help smooth out price data to identify trend direction. Crossovers between a fast (short-period) SMA and a slow (long-period) SMA provide timely entry and exit signals for trending markets. When the short SMA crosses above the long SMA, it indicates bullish momentum. Conversely, when the short SMA crosses below the long SMA, it signals bearish momentum.
...
- **Stop Loss:** Entry Price - (ATR * `stop_loss_atr_mult`). Fallback to `stop_loss_pct` if ATR is unavailable.
...
### Risk Management Integration
- **Stop Loss:** Uses ATR-based stop loss for dynamic risk control.
- **Trend Filter:** Relies on the SMA crossover logic, which inherently smooths out false signals but can be prone to whipsaws in ranging markets.
